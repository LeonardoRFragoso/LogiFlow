"""
Serviço de Envio de Emails com SendGrid
...
class SendGridEmailService:
    ...
    def send_email(
        ...
        try:
            if not self.sg:
                logger.warning("⚠️  SendGrid API key não configurada - email não será enviado")
            ...
            if response.status_code in [200, 201, 202]:
                ...
                logger.error(f"❌ Erro ao enviar email para {to_email}: Status {response.status_code}")
                return False
...
def send_welcome_email(**kwargs):
    ...
    return sendgrid_email_service.send_welcome_email(**kwargs)


def send_payment_confirmation(**kwargs):
    """Helper para enviar confirmação de pagamento"""
    return sendgrid_email_service.send_payment_confirmation(**kwargs)


def send_demo_confirmation(**kwargs):
    """Helper para enviar confirmação de demo"""
    return sendgrid_email_service.send_demo_confirmation(**kwargs)


def send_lead_notification(**kwargs):
    """Helper para notificar equipe sobre novo lead"""
    return sendgrid_email_service.send_lead_notification(**kwargs)
