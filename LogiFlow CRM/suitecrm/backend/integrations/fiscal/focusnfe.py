...
class FocusNFeClient:
    ...
    def _preparar_payload_cte(self, dados: Dict) -> Dict:
        ...
        return {
            ...
            "tipo_impressao": "1",  # DACTE Normal
            "forma_emissao": "1",  # Normal
            "ambiente": "2" if "homolog" in self.base_url else "1",
            
            # Tipo do CT-e
            "tipo_cte": "0",  # 0=Normal, 1=Complemento, 2=Anulação, 3=Substituto
            "modal": dados.get("modal", "01"),  # 01=Rodoviário
            "tipo_servico": "0",  # 0=Normal
            
            # Tomador
            "tomador": {
                "tipo": dados["tomador"]["tipo"],  # 0=Remetente, 1=Expedidor, 2=Recebedor, 3=Destinatário, 4=Outros
            ...
            "modal_rodoviario": {
                ...
                "veiculo": {
                    ...
                    "uf": dados["veiculo"]["uf"],
                    "tipo": dados["veiculo"].get("tipo", "02")  # 02=Caminhão
                }
            }
        }
    ...
    def consultar_cte(self, ref: str) -> Dict:
        ...
        Args:
            ref: Referência do CT-e na Focus NFe
            
        Returns:
            Status e dados do CT-e
        """
        try:
    ...
    def emitir_mdfe(self, dados: Dict) -> Dict:
        """
        Emite MDF-e (Manifesto de Documentos Fiscais Eletrônicos)
        
        Args:
            dados: Dicionário com dados do MDF-e
    ...
    def _preparar_payload_mdfe(self, dados: Dict) -> Dict:
        """Prepara payload MDF-e no formato Focus NFe"""
        ...
        return {
            ...
            "data_emissao": dados.get("data_emissao", datetime.now().isoformat()),
            "ambiente": "2" if "homolog" in self.base_url else "1",
            "modal": dados.get("modal", "1"),  # 1=Rodoviário
            "tipo_emitente": "1",  # 1=Prestador de serviço
            
            # Percurso
            "percurso": dados.get("percurso", []),  # Lista de UFs: ["SP", "RJ", "MG"]
    ...
    def encerrar_mdfe(self, ref: str, uf: str, cidade_codigo: str) -> Dict:
        ...
        Args:
            ref: Referência do MDF-e
            uf: UF do encerramento
            cidade_codigo: Código IBGE da cidade
